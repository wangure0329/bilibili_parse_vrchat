...
- 🎯 **專用節點**：栖隙居所專用解析節點，適配特殊需求
- 🔄 **多種解析方式**：支援智能選擇、專用節點、直接重定向三種模式
- 🎬 **簡化節點**：使用 3 個主要節點 + 1 個專用節點，確保解析結果可播放
...
- 影片連結：
  ...
  - `https://b23.tv/xxxxx` （Bilibili 短連結，自動解析）
  - `b23.tv/xxxxx` （Bilibili 短連結，自動添加 https:// 並解析）
...
- **短連結支援**：自動解析 `b23.tv` 短連結為完整 Bilibili URL
...
1. **連接到伺服器**
   ```bash
   ssh wangure0329@your-server-ip
   ```
...
**注意事項：**
- 必須按照順序執行：`cd` → `git pull` → `docker-compose restart`
- 使用 `docker-compose restart` 而不是 `down` 和 `up`，避免服務中斷
- 如需查看實時日誌，使用 `docker-compose logs -f` 命令
...
- **專用節點**: 栖隙居所欄位和 `/niche/` 路由強制使用 Mirror 節點
...
#### 栖隙居所專用解析 (`/niche/?url=BILIBILI_URL`)
1. 系統會自動解析影片
2. 獲取 1440P (2K) 品質的流地址
...
- **返回**: JSON 格式的解析結果
- **功能**: 解析影片並返回 1440P 流地址（顯示 "栖隙居所適配" 備註）
...
### v1.10.0 (2025-01-15)
- **短連結支援**: 新增 `b23.tv` 短連結自動解析功能
- **智能短連結處理**: 自動將 Bilibili 短連結解析為完整 URL 後提取 BV 號
- **前端驗證增強**: 更新 URL 驗證邏輯，支援識別 `b23.tv` 短連結格式
- **後端解析優化**: 添加短連結解析函數，支援 HTTP 重定向跟隨
- **多路由支援**: 主路由和 niche 路由均支援短連結解析

### v1.9.0 (2025-01-15)
- **URL 格式支援增強**: 支援更多 Bilibili URL 格式，包括帶參數的完整連結
- **智能 BV 號提取**: 改進正則表達式，能從複雜 URL 中正確提取 BV 號
- **錯誤處理優化**: 對不完整的 Bilibili 連結提供友好的錯誤提示
- **解析穩定性提升**: 修復函數參數問題，確保 `?url=` 參數正常重定向

### v1.8.0 (2025-01-15)
- **節點配置簡化**: 使用 3 個主要節點 + 1 個專用節點，確保解析結果可播放
- **節點替換修復**: 修復 akamaized.net 節點替換問題，支援所有 CDN 節點類型
- **解析邏輯優化**: 恢復原始解析方式，提供完整的可播放連結
- **節點選擇優化**: 簡化智能選擇邏輯，提高解析成功率
...
### v1.6.0 (2025-01-15)
- **專用節點支援**: 新增栖隙居所專用解析節點 `upos-sz-mirror08c.bilivideo.com`
- **多種解析模式**: 支援智能選擇、專用節點、直接重定向三種解析方式
- **前端專用欄位**: 新增栖隙居所專用輸入欄位，顯示 "栖隙居所適配" 備註
