...
本工具採用**隨機節點選擇策略**，支援三個主節點隨機分配：

- **主節點列表**:
  - `upos-sz-estgoss.bilivideo.com` - 主節點 (GOSS)
  - `upos-sz-estghw.bilivideo.com` - 東部節點 (GHW)
  - `upos-sz-estgcos.bilivideo.com` - 新增節點 (GCOS)
- **隨機分配**: 每次解析隨機選擇一個節點，確保負載分散
- **自動替換**: 所有其他 CDN 節點都會自動替換為選中的主節點
- **支援節點格式**:
  - `upos-sz-*.bilivideo.com` → 選中的主節點
  - `upos-hz-*.akamaized.net` → 選中的主節點
  - `upos-*-*-*.bilivideo.com` → 選中的主節點
  - `upos-*-*-*.akamaized.net` → 選中的主節點
  - `upos-*-*-*.cloudfront.net` → 選中的主節點
